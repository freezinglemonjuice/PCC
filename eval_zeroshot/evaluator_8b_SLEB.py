...
def simple_evaluate(
    ...
    if isinstance(model, str):
        ...
        class TargetLayerScaler_1(torch.nn.Module):
            def __init__(self, original_layer, scale_factor=2.0):
                super().__init__()
            ...
            def forward(self, hidden_states, attention_mask=None, position_ids=None,
                ...
                hidden_states = attn_outputs[0] + residual  # 自注意力后的残差连接
                ...
                return outputs
        class TargetLayerScaler_2(torch.nn.Module):
            def __init__(self, original_layer, scale_factor=2.0):
                super().__init__()
                self.self_attn = original_layer.self_attn
                self.mlp = original_layer.mlp
            ...
            def forward(self, hidden_states, attention_mask=None, position_ids=None,
                ...
                outputs = (hidden_states,) + attn_outputs[1:]
                return outputs
        class TargetLayerScaler_3(torch.nn.Module):
            def __init__(self, original_layer, scale_factor=2.0):
                super().__init__()
                self.self_attn = original_layer.self_attn
                self.mlp = original_layer.mlp
            ...
            def forward(self, hidden_states, attention_mask=None, position_ids=None,
                ...
                outputs = (hidden_states,) + attn_outputs[1:]
                return outputs
        def modify_model(model, target_layer_idx=4):
            original_layers = model.model.layers
            modified_layers = []
            for i, layer in enumerate(original_layers):
                if i == 8:
                    modified_layer = TargetLayerScaler_1(layer)
                if i == 18:
                    modified_layer = TargetLayerScaler_2(layer)
                if i == 19:
                    modified_layer = TargetLayerScaler_3(layer)
                else:
                    modified_layer = layer
                modified_layers.append(modified_layer)

            model.model.layers = torch.nn.ModuleList(modified_layers)
            return model
        lm._model = modify_model(lm._model)
